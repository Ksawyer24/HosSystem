...
const MedicalHistoryList = () => {
  ...
  useEffect(() => {
    const fetchMedicalHistories = async () => {
      try {
        const API_URL =
          "https://localhost:7265/api/medicalhistory/histories-total";
              ...
              {medicalHistories.map((history) => (
                ...
                >
                  ...
                  <td className="py-3 px-4">
                    <span
                      className={
                        history.hadSurgery ? "text-green-600" : "text-red-600"
                      }
                    >
                      {history.hadSurgery ? "Yes" : "No"}
                    </span>
                  </td>

                  <td className="py-3 px-4">{history?.notes ?? "N/A"}</td>
