...
const AppointmentList = () => {
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      try {
        const API_URL =
          "https://localhost:7265/api/appointments/appointments-total";
  ...
  const handleSubmit = async (e) => {
    ...
    if (!newAppointmentData.patientId)
      errors.patientId = "Patients Id is required.";

    if (!newAppointmentData.doctorId)
      errors.doctorId = "Doctors Id is required.";

    if (!newAppointmentData.appointmentDate)
      errors.appointmentDate = "Appointment Date is required.";

    if (!newAppointmentData.time) errors.time = "Time is defo  required.";

    if (!newAppointmentData.reason) errors.reason = "Reason is required.";

    if (!newAppointmentData.createdDate)
      errors.createdDate = "Date Of Appointment Creation is required.";
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setAppointments(
        appointments.filter(
          (appointment) => appointment.appointmentId !== deleteAppointmentId
        )
      );
    } catch (error) {
              ...
              {appointments.map((appointment) => (
                ...
                >
                  ...
                  <td className="py-3 px-4">
                    <span
                      className={
                        appointment.isActive ? "text-green-600" : "text-red-600"
                      }
                    >
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center overflow-auto">
            <div className="bg-white p-6 rounded shadow-lg w-full md:w-1/2 lg:w-1/3 max-h-screen overflow-y-auto">
              ...
              <form>
                ...
                />
                      {formErrors.reason &&(
                     <p className="text-red-500 text-sm mb-4">{formErrors.reason}</p>
                  )}
                ...
                />
                     ...
                     <p className="text-red-500 text-sm mb-4">{formErrors.createdDate}</p>
                  )}
              ...
              <form>
                ...
                <div className="mb-4">
                  ...
                  <input
                    ...
                    name="createdDate"
                    value={
                      new Date(editAppointmentData.createdDate)
                        .toISOString()
                        .split("T")[0]
                    }
                    onChange={handleEditChange}
